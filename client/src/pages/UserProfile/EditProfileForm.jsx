...
export const EditProfileForm = ({currentUser,setSwitch}) => {
    ...
    const handleSubmit = (e)=> {
        e.preventDefault()
        if(tags.length=== 0){
            dispatchEvent(updateProfile(currentUser?.result?.id, { name, about, tags: currentUser?.result?.tags}))
        }
        else{
            dispatchEvent(updateProfile(currentUser?.result.id, {name,about,tags}))
        }
